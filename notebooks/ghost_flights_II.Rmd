...
p1 = data.to.plot %>% 
  ...
  labs(colour="Airline", 
       size="Seat km used as % of available", 
       alpha="",
       title="") +
...
p2 = data.to.plot %>% 
  ...
  annotate("rect", xmin = as.Date("2020-04-01"), xmax = as.Date("2020-08-01"), ymin = -Inf, ymax = Inf, alpha = 0.15) +
  annotate("rect", xmin = as.Date("2021-01-01"), xmax = as.Date("2021-05-01"), ymin = -Inf, ymax = Inf, alpha = 0.15) +
...
* The 0% rule is in place, and people are emerging from lockdown 1  
* It is clear 

```{r, fig.width=5, fig.height=2}

p3 = data.to.plot %>% 
  dplyr::filter(reporting_month_year %in% c("Aug-2020", "Sep-2020", "Oct-2020", "Dec-2020")) %>% 
  ...
  scale_colour_manual(values = mycolors) +
  coord_fixed(ratio = 1) +
  facet_grid(~factor(reporting_month_year, levels=c("Aug-2020", "Sep-2020", "Oct-2020", "Dec-2020")))
...
* This is visible in the data for these airlines - seat km availability is high, showing that these airlines are scheduling many flights and % seat km used is high too, indicative that demand is meeting supply  
* As March rolls around, and COVID-19 begins to impact the UK, capacity used begins to drop off as the first UK lockdown is announced
* By April 2020, the available capacity of the airline plummets as the 80% rule is waived  
* Now that airlines can retain slots without operating at min. 80%, it is likely that during this period, airlines reduced their capacity by putting staff on furlough and scheduling far fewer flights  

## [p2]  

* Looking now to the same timecourse, but with the 2 longer UK lockdowns indicated by shading  
* In the first lockdown, we can see that some airlines (e.g. EasyJet) has low seat km available, and that this capacity is largely filled (~80% filled)
* This is in contrast to British Airways, who seem to have more available capacity, but it is under-used (~20% filled). 
* The first is that, on average, during the slot rule waiver period, flights were km was being made available, particularly between August and October of 2020, and this capacity was underused to the tune of 60-80% in some cases.  

## [p3]  

* The time between the two major UK lockdowns is interesting  
* Towards the end of this time, there is a snap lockdown for 4 weeks during November, but other than this, there are fewer restrictions in place than in the blocks of time flanking it  
* Let's zoom in on this and plot capacity available vs. capacity used for each month and each airline, to get a more granular view on this time period  
* The 0% slot rule is still in place, and yet what we see here is that one airline in particular - British Airways - has consistently more capacity than it is using  
* EasyJet is not far behind, in August and September 2020, it runs  
